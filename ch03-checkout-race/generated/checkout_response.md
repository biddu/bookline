...
```sql
-- V7__loan_open_unique.sql  (PostgreSQL)
ALTER TABLE loan ADD COLUMN IF NOT EXISTS returned_at TIMESTAMP NULL;

-- At most ONE open loan (returned_at IS NULL) per copy, enforced by the DB.
CREATE UNIQUE INDEX IF NOT EXISTS ux_loan_open_copy
    ON loan (copy_barcode)
